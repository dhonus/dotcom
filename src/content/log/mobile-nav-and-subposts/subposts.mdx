...
- The breadcrumb component required careful thought to handle three distinct cases:
    ...
    ```astro title="src/pages/log/[...id].astro" startLineNumber={70}
    <Breadcrumbs
      items={[
        { href: '/log', label: 'Log', icon: 'lucide:library-big' },
        ...(isCurrentSubpost && parentPost
          ? [
              {
                href: `/log/${parentPost.id}`,
                label: parentPost.data.title,
                icon: 'lucide:book-open',
              },
              {
                href: `/log/${currentPostId}`,
...
- The main log listing (alongside other listings, e.g. filtering by tags, filtering by author) should exclude subposts to avoid cluttering the feed:
    ...
    Without this filter, your log listing would show every subpost as a top-level entry, defeating the purpose of hierarchical organization.
